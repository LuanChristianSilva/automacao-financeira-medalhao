...
def transform_silver():
    ...
    logger = PipelineLogger(
        ...
        log_file_path=os.path.join("logs", "silver_transform_log.csv")
    )
    logger.start("Iniciando processamento DuckDB (Bronze -> Silver)")
    ...
    try:
        ...
        # Extrai metricas para log (Conta quantitativo salvo)
        linhas_renda = con.execute("SELECT COUNT(*) FROM silver_renda").fetchone()[0]
        linhas_despesa = con.execute("SELECT COUNT(*) FROM silver_despesa").fetchone()[0]
        
        logger.log_metric("Silver Renda", "rows_transformed_renda", linhas_renda, metric_unit="rows", row_count=linhas_renda)
        logger.log_metric("Silver Despesa", "rows_transformed_despesa", linhas_despesa, metric_unit="rows", row_count=linhas_despesa)

        con.close()
        logger.finish("Transformação Silver gerada com sucesso.")

    except Exception as e:
        logger.log_error(f"Erro na transformação Silver: {str(e)}")
        raise e
